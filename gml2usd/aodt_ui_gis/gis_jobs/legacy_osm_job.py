...
def run_aodt_gis(in_usd):

    cmd_str = f"{aodt_legacy_gis_location} {in_usd} -o {tmp_path2} -v info --flatten"
...
class OsmJob:
    ...
    def run(self):
        ...
        if utils.bb_is_valid_and_acceptable(self, 100):
            ...
            in_usd = tmp_path
            utils.copy_tmp_template(template_location, tmp_path2)
            run_aodt_gis(in_usd)
            utils.add_default_materials(tmp_path2)
            utils.copy(tmp_path2, self.__getattribute__("output_stage"))

            try:
                # cleanup
                subprocess.call(f"rm -r {tmp_path2}", shell=True)
            except:
                pass

            #try:
            #    # cleanup
            #    subprocess.call(f"rm -r {tmp_textures_path2}", shell=True)
            #except:
            #    pass

            try:
                # cleanup
                subprocess.call(f"rm -r {in_usd}", shell=True)
            except:
                pass
